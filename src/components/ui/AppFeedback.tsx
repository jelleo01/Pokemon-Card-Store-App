...
export default function AppFeedback() {
  ...
  async function submit() {
    ...
    try {
      const { error } = await supabase.rpc('card_submit_feedback', { stars: rating, summary: feedback.trim(), details: feedback.trim() })
      if (error) throw error
      setSaved(true)
  ...
  return <PixelBorder padding={16} style={{ flexShrink: 0, margin: '8px 0 20px' }}>
    <b>앱 사용 후기</b>
    <p style={{ fontSize: 12, lineHeight: 1.8, margin: '8px 0 16px' }}>별점과 한줄평(피드백)을 남겨 주세요.<br />별점에 관계없이 최초 1회 <b>+15 P</b> · 운영자에게만 전달돼요.</p>
    {saved || existing.data ? <div role="status"><b>후기를 남겨 주셔서 감사합니다!</b>{existing.data && <p style={{ marginTop: 8, fontSize: 13 }}>{'★'.repeat(existing.data.rating)} · {existing.data.headline}</p>}</div>
      ...
      : <form onSubmit={e => { e.preventDefault(); void submit() }} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <label>한줄평(피드백)<input required disabled={busy} value={feedback} onChange={e => setFeedback(e.target.value)} minLength={5} maxLength={100} placeholder="좋았던 점이나 개선할 점을 5자 이상 알려 주세요." style={inputStyle} /></label>
        {error && <p role="alert" style={{ color: 'var(--red)', fontSize: 13 }}>{error}</p>}
        <PixelButton type="submit" full disabled={busy || !rating || feedback.trim().length < 5}>{busy ? '저장 중…' : '제출하고 +15 P'}</PixelButton>
      </form>}
  </PixelBorder>
}
